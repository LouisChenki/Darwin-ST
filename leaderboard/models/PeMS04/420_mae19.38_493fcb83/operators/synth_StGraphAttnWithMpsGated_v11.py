...
class StGraphAttnWithMpsGated_v11(nn.Module):
    """Improved version of synth_StGraphAttnWithMpsGated_v10 and synth_StGraphAttnWithMpsGated_v7.
    
    V11 builds on v10's strong gate-on-input design, but enriches the gate by conditioning on both
    the original input x and the post-attention features h, capturing complementary information.
    The gate is a small MLP with GELU, taking concatenated [x, h] and outputting a channel-wise
    gating mask. All other components (ST-Attn, MPS bottleneck) are kept identical to v10.
    """
    def __init__(self, channels, num_nodes, **kw):
        ...
        # Richer gate: operates on [x, h] (2*channels) -> hidden -> channels -> sigmoid
        gate_input_dim = 2 * channels
        gate_hidden = max(1, channels // 2)  # moderate width
    ...
    def forward(self, x, adj=None):
        ...
        # 3. Dynamic gate conditioned on both raw input x and transformed h
        gate_input = torch.cat([x, h], dim=-1)    # [B, T, N, 2*C]
