# 模型卡: darwin-st/18e8a1c0

- **数据集**: PeMS04
- **排名**: 第 170 名 / 库内 KEEP 实验
- **val MAE**: 18.62
- **val RMSE**: 30.71
- **参数量**: 221.8K
- **实验 id**: 567   **签名**: `18e8a1c0cd4d84ea407223f78f0003922ad5005d`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-04T12:48:08.261857+00:00
...
- 时空块:
  ...
  1. spatial=`synth_StGraphAttnWithMpsGated_v20` temporal=`st_graph_attn` fusion=`cross` 🔬
  2. spatial=`adaptive` temporal=`st_separable` fusion=`cross`
  3. spatial=`stjoint_conv` temporal=`identity` fusion=`cross`
...
### `synth_StGraphAttnWithMpsGated_v20`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.79
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v20.py`](operators/synth_StGraphAttnWithMpsGated_v20.py)
