# 模型卡: darwin-st/2a2df1e5

- **数据集**: PeMS04
- **排名**: 第 142 名 / 库内 KEEP 实验
- **val MAE**: 18.57
- **val RMSE**: 30.75
- **参数量**: 271.3K
- **实验 id**: 650   **签名**: `2a2df1e5c6a35564208919cdbbc78ca57de3c4e6`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-06T08:44:55.094463+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +0.77
- **评测协议**: masked metric; MAE averaged over 12 horizons; split 6/2/2

## 架构

- 深度 (blocks): 7   hidden: 64   邻接: rw
- 时空块:
  0. spatial=`synth_StGraphAttnWithMpsGated_v7` temporal=`identity` fusion=`cross` 🔬
  1. spatial=`synth_StGraphAttnWithMpsGated_v8` temporal=`st_graph_attn` fusion=`cross` 🔬
  2. spatial=`dynamic_gat` temporal=`identity` fusion=`cross`
  3. spatial=`synth_InfoBootstrapResidual` temporal=`stjoint_conv` fusion=`cross` 🔬
  4. spatial=`synth_StGraphAttnWithMpsGated_v4` temporal=`synth_InfoBootstrapResidual` fusion=`residual` 🔬
  5. spatial=`cheb` temporal=`gru` fusion=`cross`
  6. spatial=`synth_StGraphAttnWithMpsGated_v3` temporal=`synth_StGraphAttnWithMpsGated_v23` fusion=`residual` 🔬
- 身份嵌入: node=True(128) tod=True(64) dow=True(64)

## 🔬 跨域合成算子 (LLM 创造)

本模型用到以下由 LLM 跨域类比合成、经验证门通过的算子:

### `synth_StGraphAttnWithMpsGated_v7`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 46.55
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v7.py`](operators/synth_StGraphAttnWithMpsGated_v7.py)

### `synth_StGraphAttnWithMpsGated_v8`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.96
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v8.py`](operators/synth_StGraphAttnWithMpsGated_v8.py)

### `synth_InfoBootstrapResidual`
- **组合方式**: additive_residual
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 19.14
- **算子代码**: [`operators/synth_InfoBootstrapResidual.py`](operators/synth_InfoBootstrapResidual.py)

### `synth_StGraphAttnWithMpsGated_v4`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.94
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v4.py`](operators/synth_StGraphAttnWithMpsGated_v4.py)

### `synth_StGraphAttnWithMpsGated_v3`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 19.22
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v3.py`](operators/synth_StGraphAttnWithMpsGated_v3.py)

### `synth_StGraphAttnWithMpsGated_v23`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.70
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v23.py`](operators/synth_StGraphAttnWithMpsGated_v23.py)

## 超参 (HPO 择优)

```json
{
  "lr": 0.007098936257405904,
  "weight_decay": 1.6334587611069488e-06,
  "dropout": 0.043564649850770354,
  "batch_size": 32,
  "lr_schedule": "cosine",
  "num_heads": 2
}
```

## 复现

- 架构: [`genotype.json`](genotype.json) → `Genotype.from_dict(...)` → `build_model(...)`
- 超参: [`hparams.json`](hparams.json)
- 权重存档: [`model.pt`](model.pt) (训练最优 val-MAE 步的 state_dict, 附 sidecar 元数据 `.meta.json`)
- 合成算子: `operators/*.py` 经 `OperatorRegistry.load_persisted()` 注入后方可构建
