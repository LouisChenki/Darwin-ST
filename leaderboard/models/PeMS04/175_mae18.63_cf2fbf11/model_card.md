# 模型卡: darwin-st/cf2fbf11

- **数据集**: PeMS04
- **排名**: 第 175 名 / 库内 KEEP 实验
- **val MAE**: 18.63
- **val RMSE**: 30.71
- **参数量**: 234.7K
- **实验 id**: 336   **签名**: `cf2fbf11dd852d0a29453e69282f7c6ca00cf2a1`
- **run_tag**: exp/tier2-v1   **时间**: 2026-07-30T22:40:44.708130+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +0.83
- **评测协议**: masked metric; MAE averaged over 12 horizons; split 6/2/2

## 架构

- 深度 (blocks): 7   hidden: 64   邻接: sym
- 时空块:
  0. spatial=`synth_StGraphAttnWithMpsGated_v9` temporal=`synth_StGraphAttnWithMpsGated_v12` fusion=`cross` 🔬
  1. spatial=`st_separable` temporal=`st_graph_attn` fusion=`cross`
  2. spatial=`adaptive` temporal=`dynamic_gat` fusion=`cross`
  3. spatial=`adaptive` temporal=`identity` fusion=`cross`
  ...
  6. spatial=`synth_st_graph_attn_with_vib_compression` temporal=`ssm` fusion=`residual` 🔬
...
### `synth_StGraphAttnWithMpsGated_v12`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.95
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v12.py`](operators/synth_StGraphAttnWithMpsGated_v12.py)
