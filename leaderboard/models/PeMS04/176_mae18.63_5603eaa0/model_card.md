# 模型卡: darwin-st/5603eaa0

- **数据集**: PeMS04
- **排名**: 第 176 名 / 库内 KEEP 实验
- **val MAE**: 18.63
- **val RMSE**: 30.72
- **参数量**: 199.1K
- **实验 id**: 408   **签名**: `5603eaa0cf4f5013e27bf7ebffbe822338acfad7`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-01T05:51:53.504905+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +0.83
...
- 时空块:
  0. spatial=`synth_StGraphAttnWithMpsGated_v17` temporal=`identity` fusion=`cross` 🔬
  1. spatial=`st_separable` temporal=`st_graph_attn` fusion=`cross`
  2. spatial=`synth_StGraphAttnWithMpsGated_v10` temporal=`st_separable` fusion=`cross` 🔬
...
### `synth_StGraphAttnWithMpsGated_v17`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 19.47
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v17.py`](operators/synth_StGraphAttnWithMpsGated_v17.py)

### `synth_StGraphAttnWithMpsGated_v10`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.73
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v10.py`](operators/synth_StGraphAttnWithMpsGated_v10.py)
