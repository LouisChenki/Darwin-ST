# 模型卡: darwin-st/8c201631

- **数据集**: PeMS04
- **排名**: 第 328 名 / 库内 KEEP 实验
- **val MAE**: 18.99
- **val RMSE**: 31.05
- **参数量**: 192.6K
- **实验 id**: 144   **签名**: `8c2016310c2cbf7a3b68105400987947e102edaa`
- **run_tag**: exp/tier2-v1   **时间**: 2026-07-27T08:57:07.580724+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +1.19
...
- 时空块:
  0. spatial=`synth_StGraphAttnWithMpsGated_v4` temporal=`st_graph_attn` fusion=`cross` 🔬
  1. spatial=`st_separable` temporal=`dynamic_gat` fusion=`cross`
  2. spatial=`st_graph_attn` temporal=`st_separable` fusion=`cross`
  3. spatial=`synth_InfoBootstrapResidual` temporal=`synth_StGraphAttnWithMpsGated_v2` fusion=`parallel` 🔬
