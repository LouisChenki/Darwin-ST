# 模型卡: darwin-st/f032f26a

- **数据集**: PeMS04
- **排名**: 第 199 名 / 库内 KEEP 实验
- **val MAE**: 18.67
- **val RMSE**: 30.81
- **参数量**: 222.3K
- **实验 id**: 521   **签名**: `f032f26a2e0b2107228e38e513d550a42feae2a1`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-03T13:02:28.399821+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +0.87
...
- 时空块:
  ...
  2. spatial=`adaptive` temporal=`series_decomp_attn` fusion=`cross`
  3. spatial=`dynamic_gat` temporal=`identity` fusion=`cross`
  4. spatial=`synth_InfoBootstrapResidual` temporal=`stjoint_conv` fusion=`iterative` 🔬
  5. spatial=`identity` temporal=`synth_InfoBootstrapResidual` fusion=`residual` 🔬
  6. spatial=`st_graph_attn` temporal=`synth_StGraphAttnWithMpsGated_v3` fusion=`residual` 🔬
