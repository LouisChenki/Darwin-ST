...
class StGraphAttnWithMpsGated_v9(nn.Module):
    """Improved version of synth_StGraphAttnWithMpsGated_v4 and synth_StGraphAttnWithMpsGated_v7.
    
    V9 retains the stable gating philosophy from v4 (gate derived from input x with
    LayerNorm), but upgrades the gate network to a small non‑linear MLP for richer
    routing decisions. All core components (ST‑Attention, MPS bottleneck) are kept
    unchanged, and the gate still operates on x to avoid the performance drop seen
    in v7.
    """
    def __init__(self, channels, num_nodes, **kw):
        ...
        # v9 gate: two‑layer MLP with LayerNorm for richer conditioning on x
        gate_hidden = max(1, channels // 4)
    ...
    def forward(self, x, adj=None):
        ...
        # 3. Dynamic gate from input statistics (v9: richer MLP, still on x)
