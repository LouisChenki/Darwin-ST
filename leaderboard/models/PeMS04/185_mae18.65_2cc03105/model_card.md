# 模型卡: darwin-st/2cc03105

- **数据集**: PeMS04
- **排名**: 第 185 名 / 库内 KEEP 实验
- **val MAE**: 18.65
- **val RMSE**: 30.66
- **参数量**: 257.9K
- **实验 id**: 652   **签名**: `2cc03105f4a1380a2868df965dea6661efa9987e`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-06T09:18:32.973960+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +0.85
...
- 时空块:
  ...
  3. spatial=`dynamic_gat` temporal=`identity` fusion=`sequential_ts`
  4. spatial=`synth_InfoBootstrapResidual` temporal=`stjoint_conv` fusion=`cross` 🔬
  5. spatial=`synth_StGraphAttnWithMpsGated_v4` temporal=`synth_InfoBootstrapResidual` fusion=`cross` 🔬
  6. spatial=`cheb` temporal=`gru` fusion=`cross`
