# 模型卡: darwin-st/69a16338

- **数据集**: PeMS04
- **排名**: 第 424 名 / 库内 KEEP 实验
- **val MAE**: 19.43
- **val RMSE**: 31.67
- **参数量**: 300.1K
- **实验 id**: 632   **签名**: `69a16338545c72afdd692732d104f1fc3f1000a7`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-05T23:30:25.970040+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +1.63
...
- 时空块:
  ...
  1. spatial=`synth_StGraphAttnWithMpsGated_v30` temporal=`st_graph_attn` fusion=`cross` 🔬
  2. spatial=`adaptive` temporal=`st_separable` fusion=`cross`
  3. spatial=`dynamic_gat` temporal=`identity` fusion=`cross`
  4. spatial=`synth_InfoBootstrapResidual` temporal=`stjoint_conv` fusion=`residual` 🔬
  ...
  7. spatial=`mixhop` temporal=`attn` fusion=`sequential_ts`
...
### `synth_StGraphAttnWithMpsGated_v30`
- **组合方式**: gated_routed
- **源机制 (跨域)**: minimal_predictive_sufficiency
- **该算子实测 MAE**: 18.63
- **算子代码**: [`operators/synth_StGraphAttnWithMpsGated_v30.py`](operators/synth_StGraphAttnWithMpsGated_v30.py)
