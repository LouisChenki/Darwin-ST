# 模型卡: darwin-st/850ac647

- **数据集**: PeMS04
- **排名**: 第 408 名 / 库内 KEEP 实验
- **val MAE**: 19.30
- **val RMSE**: 31.35
- **参数量**: 289.1K
- **实验 id**: 574   **签名**: `850ac64721e76b76417183f4007e798319ec0864`
- **run_tag**: exp/tier2-v1   **时间**: 2026-08-04T15:46:34.548846+00:00
- **vs SOTA** (STD-MAE 17.80): 距 SOTA 差 +1.50
...
- 时空块:
  0. spatial=`synth_StGraphAttnWithMpsGated_v23` temporal=`identity` fusion=`cross` 🔬
  1. spatial=`st_separable` temporal=`st_graph_attn` fusion=`cross`
  2. spatial=`adaptive` temporal=`st_separable` fusion=`cross`
  3. spatial=`synth_StGraphAttnWithMpsGated_v9` temporal=`identity` fusion=`cross` 🔬
...
- 身份嵌入: node=True(128) tod=True(128) dow=True(64)
