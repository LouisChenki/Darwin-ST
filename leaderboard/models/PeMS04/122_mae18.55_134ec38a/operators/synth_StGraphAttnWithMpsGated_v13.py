...
class StGraphAttnWithMpsGated_v13(nn.Module):
    """Improved version of synth_StGraphAttnWithMpsGated_v10 and synth_StGraphAttnWithMpsGated_v7.

    V13 combines the strengths of v10 (rich gate MLP with GELU, stable input-conditioning)
    and avoids the simplicity of v7's post‑attention gate.
    Gate now sees both raw input x and post‑attention features h, allowing context‑aware
    blending while keeping the deeper MLP architecture. Residual scale alpha stays zero‑init.
    """
    def __init__(self, channels, num_nodes, **kw):
        ...
        # Gate: richer MLP on concatenation of x and h, reuse v10's width and activation
        gate_hidden = max(1, channels // 2)
        self.gate_net = nn.Sequential(
            nn.LayerNorm(channels * 2),               # norm over concatenated C+C
    ...
    def forward(self, x, adj=None):
        ...
        # 3. Dynamic gate from both raw input x and post‑attention features h
