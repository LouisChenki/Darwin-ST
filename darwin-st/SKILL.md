...
2. **强制文献检索 (Mandatory GraphRAG Query)**:
   作为严谨的学术研究员，**在每一轮（每一个 Epoch 修改前）都必须绝对强制**向知识图谱请求理论支撑，绝不允许凭空瞎想：
   - 必须先输出公开声明（如：`"📡 [Darwin-ST 知识库请求]: <陈述当前瓶颈或下一轮构思>正在向文献图谱请求处方..."`）
   - 随后，**必须**使用终端执行文献检索工具：`python3 scripts/graph_rag.py` （使用 `--action diagnose` 针对错误寻药，或用 `--action verify` 验证新点子）。
3. **输出融合优化方案 (Formulate Optimization Plan)**:
   - 收到 `graph_rag.py` 的返回内容（数学机制与 Anti-pattern 教训）后，**严禁立即动手改代码**。
   - 必须在继续执行前，向终端/对话外显地打印一段简短的【优化规划 (Optimization Design)】。要求在此规划中，深思熟虑地阐述：如何将新检索到的文献机制与 `memory.md` 中已经记录的有效机制进行有机融合，以防止教条主义导致原核心破坏。
4. **执行并计时突变**: 运行根据【优化规划】修改后的 `python3 train.py`。该脚本必须受限于 20 分钟的墙上时钟计算限额。
