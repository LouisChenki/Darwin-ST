...
1. **读取海马体现存记忆**: 每次修改 `train.py` 之前，必须读取当前目录下的 `memory.md` 和 `results.tsv`，纵览全局历史坐标。充分理解当前记录在 DNA 里的 SOTA 基因，并绕开失败墓地里的死路。
2. **强制文献基站检索 (Mandatory GraphRAG Query - INSPIRATION SUPERMARKET)**:
   作为严谨的学术研究员，**在开启每一轮全新的模型变异 (Mutation) 修改代码之前，你被绝对禁止凭空凭直觉硬想！** GraphRAG 是你的灵感缪斯，请强制基于以下优先级执行终端调用 (`run_command`: `python3 scripts/graph_rag.py`)：
   - **优先级 1 (Primary - 汲取创意)**: 如果上一轮正常推进，但前路未知、需要打破局部 SOTA 时。强制执行 `--action inspire`，系统将随机抛给你一批跨度极大的方法家族和模块概念（包括基础层与前沿层）。请发挥你的跨界融合力从中挑选并生成本轮突变点。
   - **优先级 2 (Secondary - 验证与排雷)**: 如果你脑子里对某些明确的新架构成分（比如自适应图谱注意力等）已经有了非常成熟的构思，请用 `--action verify --concept "..."` 查看图谱中关于此构思的经验与教训。
   - **优先级 3 (Secondary - 对症下药)**: 只有在你碰到了切实的算法缺陷（例如过平滑阻碍、时序依赖丢失等）时，再调用 `--action diagnose --symptom "..."` 寻医问药。
   - **⚠️ 纪律红线**: 在你的对话区看到 `graph_rag.py` 吐出的结果之前，**绝对严禁**直接跳到下一步或改写 `train.py`！调用前应当先输出类似于 `"📡 [Darwin-ST 知识库请求]: <陈述目的>..."` 的声明。
3. **出具纯文本【优化规划】(Formulate Optimization Plan)**:
   - 收到 `graph_rag.py` 的文献教训与 Anti-pattern 后，**还是不准立即去改代码**。
   - 必须停下来，先在黑框里正儿八经地打印输出一份简短但深刻的【优化规划 (Optimization Design)】。在方案中写明：“我计划如何将 RAG 供给的高级数学机制，与 `memory.md` 中已经踩紧的地基 (有效代码) 进行跨越式的化学融合。”
4. **执行并强力管控突变训练**: 运行根据【优化规划】生成的 `python3 train.py`。为了防止你在代码里用短 Epoch 敷衍了事，**你的代码必须服从以下训练纪律**：
   - **防敷衍代码锁**: 编写代码时，绝对不允许把 `epochs` 设置为 5、10 等极短敷衍数字。`epochs` 在代码层面至少设为 `50`。
   - **强制时间熔断器**: 为了受限于 20 分钟单次突变算力限额，并且依然能让模型在这个时间窗口内尽可能收敛，你必须在 `train.py` 的每一个 `epoch` 结尾加入基于真实时钟的时空监控阻断代码（如 `if time.time() - start_time > 1100: break`）。
   - **⚠️ NaN 极速熔断机制**: 在向终端发送执行命令后，你必须持续监控输出的日志流。哪怕代码在跑，一旦发现有 `Loss=nan` 或 `Val MAE=inf`，**你必须立即用 `^C` 杀死该进程，不准眼睁睁等它跑完！** 这属于灾难崩溃直接进入 (DISCARD) 步骤。
5. **贝叶斯后验反思**:
...
6. **封杀礼貌性停顿 (Banning Polite Interruptions)**:
...
7. **终极使命：无尽追寻绝对 SOTA (Endless Pursuit of Absolute SOTA)**:
...
8. **批处理与内存安全声明 (Batch Maintenance Declaration)**:
